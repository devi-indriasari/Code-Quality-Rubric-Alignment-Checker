...
def predict_with_model(text: str):
    ...
    if hasattr(model, "predict_proba"):
        ...
        probability_df = (
            pd.DataFrame({"Label": [canonicalize_label(label) for label in class_labels], "Probabilitas": probabilities})
...
sidebar_card(
    "Label Model",
    "Variables, Expressions, Control Flow, Comments, Layout and Formatting, Decomposition, dan General.",
)
...
with left_col:
    ...
    with st.expander("Cara kerja sistem"):
        st.markdown(
            """
            1. Mahasiswa memilih file kode Java dari folder `code`.
            ...
            5. Hasil review otomatis disimpan ke folder `review` dalam format JSON dan CSV.
            6. Jika belum sesuai, mahasiswa mendapat saran perbaikan.
            """
        )
...
if st.session_state.get("has_prediction_result") and st.session_state.get("last_result_df") is not None:
    ...
    st.caption(
        ...
        "agar komentar mahasiswa lebih spesifik dan sesuai dengan kriteria rubrik. Hasil prediksi bisa salah. "
        "Jika Anda tetap ingin submit meskipun ada prediksi yang belum sesuai, tekan tombol Submit Anyway."
    )
    ...
    if st.session_state.get("peer_review_submitted"):
        ...
        st.markdown(
            ...
            <div class="success-submit-card">
                ...
                Feedback terhadap kode yang dinilai telah tersimpan di folder review. Pengiriman dilakukan setelah mahasiswa melihat hasil prediksi sistem.
            </div>
            """,
            unsafe_allow_html=True,
        )
