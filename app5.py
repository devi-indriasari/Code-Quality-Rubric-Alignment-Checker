...
def _flatten_score_vector(scores, class_labels: list[str]) -> np.ndarray | None:
    """Flatten model scores into one value per class label."""
    if scores is None:
        return None

    scores = np.asarray(scores, dtype=float)

    # Multiclass models usually return shape (1, n_classes).
    if scores.ndim > 1:
        scores = scores[0]

    scores = np.asarray(scores, dtype=float).reshape(-1)

    # Binary classifiers sometimes return one margin score. Convert it to two margins.
    if scores.size == 1 and len(class_labels) == 2:
...
def _softmax_from_score_vector(scores) -> np.ndarray | None:
    """Convert a vector of model scores to relative percentages that sum to 1."""
    if scores is None:
        return None
...
def _build_probability_df(
    class_labels: list[str],
    values,
    score_source: str,
    raw_scores=None,
) -> pd.DataFrame | None:
    """Build a complete per-label score table for one comment.

    For LinearSVC, `values` contains softmax-normalized decision scores.
    They are displayed as relative confidence percentages, not calibrated
    statistical probabilities. The table is built for one comment/input only,
    so the percentages in one table sum to about 100%.
    """
    if values is None:
        return None
    ...
    if raw_scores.size != values.size:
        raw_scores = np.full(values.size, np.nan)

    df = pd.DataFrame({
        "Label": [canonicalize_label(label) for label in class_labels],
        "Probabilitas": values,
        "Persentase": values * 100,
        "Persentase (%)": [f"{value * 100:.2f}%" for value in values],
        "Skor Model": raw_scores,
        "Sumber Skor": score_source,
    })

    return df.sort_values("Probabilitas", ascending=False).reset_index(drop=True)
...
def predict_with_model(text: str):
    ...
    if hasattr(model, "predict_proba"):
        ...
        raw_decision_scores = _flatten_score_vector(
            model.decision_function([model_input]),
            class_labels,
        )
        confidence_scores = _softmax_from_score_vector(raw_decision_scores)
        probability_df = _build_probability_df(
            class_labels=class_labels,
            values=confidence_scores,
            score_source="decision_function_softmax",
            raw_scores=raw_decision_scores,
        )
...
def demo_predict(text: str):
    ...
    labels = list(scores.keys()) + ["General"]
    raw_scores = np.array([scores[label] for label in scores] + [1 if best_label == "General" else 0], dtype=float)
    total = raw_scores.sum()
    values = raw_scores / total if total > 0 else np.zeros_like(raw_scores)

    probability_df = _build_probability_df(
        class_labels=labels,
        values=values,
        score_source="demo_keyword_score",
...
def get_score_source(probability_df: pd.DataFrame | None) -> str:
    ...
    if source == "decision_function_softmax":
        return "estimasi dari decision score LinearSVC"
    if source == "demo_keyword_score":
        return "demo keyword score"
    return source
...
def format_model_score(score) -> str:
    try:
...
def probability_detail_records(probability_df: pd.DataFrame | None) -> list[dict]:
    """Convert the complete per-label score table into JSON-safe records."""
    if probability_df is None or probability_df.empty:
        return []

    records = []
    for _, row in probability_df.iterrows():
        probability = row.get("Probabilitas", None)
        percentage = row.get("Persentase", None)
        score = row.get("Skor Model", None)
        try:
            probability_value = None if probability is None or pd.isna(probability) else float(probability)
        except (TypeError, ValueError):
            probability_value = None
        try:
            percentage_value = None if percentage is None or pd.isna(percentage) else float(percentage)
        except (TypeError, ValueError):
        ...
        records.append({
            ...
            "Probabilitas": probability_value,
            "Persentase": percentage_value,
            "Persentase (%)": row.get("Persentase (%)", format_probability(probability_value)),
            "Skor Model": score_value,
            "Skor Model (format)": format_model_score(score_value),
            "Sumber Skor": str(row.get("Sumber Skor", "")),
        })
    return records


def probability_detail_json(probability_df: pd.DataFrame | None) -> str:
    return json.dumps(probability_detail_records(probability_df), ensure_ascii=False)


def probability_detail_dataframe(detail_json: str | list | None) -> pd.DataFrame:
    if detail_json is None:
        return pd.DataFrame()

    if isinstance(detail_json, list):
        records = detail_json
    else:
        try:
            records = json.loads(str(detail_json))
    ...
    if "Skor Model" in df.columns:
        df["Skor Model"] = pd.to_numeric(df["Skor Model"], errors="coerce")
    return df


def render_label_percentage_visualization(detail_df: pd.DataFrame, chart_key: str):
    """Render a per-label percentage chart and table for one field/comment."""
    if detail_df is None or detail_df.empty:
        st.info("Detail persentase semua label tidak tersedia untuk komentar ini.")
        return

    chart_df = detail_df[["Label", "Persentase"]].copy()
    chart_df = chart_df.dropna(subset=["Persentase"])
    chart_df = chart_df.set_index("Label")

    if not chart_df.empty:
        st.bar_chart(chart_df, use_container_width=True)

    display_columns = ["Label", "Persentase (%)", "Skor Model (format)"]
    display_df = detail_df.copy()
    if "Skor Model (format)" not in display_df.columns and "Skor Model" in display_df.columns:
        display_df["Skor Model (format)"] = display_df["Skor Model"].apply(format_model_score)
    display_df = display_df[[column for column in display_columns if column in display_df.columns]]
    display_df = display_df.rename(columns={"Skor Model (format)": "Decision Score"})
    st.dataframe(display_df, use_container_width=True)
    st.caption(
        "Grafik ini hanya berlaku untuk komentar pada field ini. Total persentase di dalam satu grafik mendekati 100%. "
        "Jangan menjumlahkan persentase antarfield karena setiap field diprediksi dari komentar yang berbeda."
    )
...
sidebar_card(
    "Label Model",
    "Variables, Expressions, Control Flow, Comments, Layout and Formatting, Decomposition, dan General. Setiap hasil field menampilkan visualisasi persentase semua label agar mahasiswa dapat melihat kecenderungan alternatif prediksi.",
)
...
with left_col:
    ...
    with st.expander("Cara kerja sistem"):
        st.markdown(
            ...
            4. Sistem menampilkan label prediksi, persentase label terpilih, visualisasi persentase semua label, dan status kesesuaian dengan field yang diisi.
    ...
    with st.spinner("Sistem sedang memprediksi klasifikasi komentar dan mengecek kesesuaian field..."):
        for item in RUBRIC_FIELDS:
            ...
            results.append({
                ...
                "Sumber Skor": score_source,
                "Detail Persentase Semua Label": probability_detail_json(probability_df),
...
if st.session_state.get("has_prediction_result") and st.session_state.get("last_result_df") is not None:
    ...
    if all_fields_match:
        st.success("Semua komentar terdeteksi sesuai dengan kriteria rubrik. Tetap baca ulang label prediksi dan visualisasi persentase semua label karena sistem ML tidak menjamin kebenaran 100%.")
    else:
        st.warning(
            "Beberapa komentar belum terdeteksi sesuai dengan kriteria field. "
            "Cek label prediksi, visualisasi persentase semua label, dan teks yang masuk ke sistem. Prediksi ML bisa salah, sehingga keputusan akhir tetap memerlukan validasi manusia."
        )
    ...
    for _, row in result_df.iterrows():
        ...
        st.markdown(
            ...
            <div class="summary-card {card_class}">
                ...
                <div class="summary-line"><b>Persentase label terpilih:</b> {safe_probability}</div>
        ...
        detail_df = probability_detail_dataframe(row.get("Detail Persentase Semua Label", "[]"))
        with st.expander(f"Visualisasi persentase semua label - {row['Kriteria Field']}", expanded=True):
            render_label_percentage_visualization(detail_df, chart_key=f"chart_{row['Field Key']}")
    ...
    st.caption(
        ...
        "dan tetap perlu divalidasi oleh manusia. Karena model terbaik menggunakan LinearSVC, persentase pada setiap field dihitung "
        "dari decision score yang dinormalisasi dengan softmax. Grafik persentase semua label hanya berlaku untuk satu komentar pada field tersebut; "
        "jangan menjumlahkan persentase antarfield karena setiap field diprediksi dari komentar yang berbeda. "
